...
$(document).ready(function(){foodly.cacheSelectors();foodly.coverImg('.js-img-holder-cover');foodly.init();});$(window).load(function(){$('.content-wrapper').css({'overflow':'hidden'});});var foodly={cacheSelectors:function(){this.cache={$body:$('body'),$cart:$('#cart'),$header:$('.header'),modalScroll:[]};},init:function(){this.updateDropdownCart();this.initHeader();this.initAddToCard('form[action*="cart/add"]');this.initChangeQuantity('form[action*="cart"]');this.initUpdateCartAjax();this.initRecoverPasswordForm('.login-form');this.initFormsError();this.showTopBar('.fixed-bar');this.initDropdown('.js-dropdown');
this.initModalAddresses();this.initDropdownScroll('.js-dropdown-scroll');this.foldElements('.fold');this.showHideMaps('.map-wrapper');this.initToolbar('.collection__toolbar');this.initAddToWishlist('.js-wishlist-form');this.initActionsWishlist('.js-wishlist');this.initDropdownAfterChanageSelector('#address_country','#address_province');this.initDropdownAfterChanageSelector('#AddressCountryNew','#AddressProvinceNew');this.initDropdownAfterChanageSelector('.AddressCountry','.AddressProvince');this.parallaxScroll('.parallax');},initHeader:function(){this.drawerOpenClose('.drawer--menu');this.drawerOpenClose('.drawer--cart');this.collapseSubmenu('.drawer--menu');var $overlay=$('<div/>').addClass('overlay');$overlay.click(function(){$this.closeDrawer($('.drawer'));});this.cache.$header.append($overlay);this.searchHeaderToggle();this.fixedHeader('.header');this.cache.$body.mousedown(function(e){$target=$(e.target);$parentTargetDropdown=$target.closest('.dropdown');$openDropdown=$('.dropdown.open');$parentTargetSearch=$target.closest('.search-container');$openSearch=$('.search-container.open');if($parentTargetDropdown.length===0&&$openDropdown.length>0){$openDropdown.removeClass('open');}else if($parentTargetSearch.length===0&&$openSearch.length>0){$openSearch.removeClass('open');$openSearch.children().removeClass('popup-active');}});},drawerOpenClose:function(drawer){var $drawer=$(drawer);if($drawer.length===0){return 0;}
var $drawerBtnOpen=$drawer.find('.drawer__btn-open');var $drawerBtnClose=$drawer.find('.drawer__btn-close');var $drawerWrapper=$drawer.find('.drawer__wrapper');var $parallaxBack=this.cache.$body.find('.parallax__back');var $parallaxBase=this.cache.$body.find('.parallax__base');var $header=this.cache.$body.find('.header');var $this=this;$drawerBtnOpen.click(function(e){e.preventDefault();if($(window).scrollTop()>0){$('html, body').animate({scrollTop:0},1000,function(){setTimeout(function(){$this.openDrawer($drawer);$this.dropParallaxCoords($parallaxBack,$parallaxBase);},450);});}else{$this.openDrawer($drawer);$this.dropParallaxCoords($parallaxBack,$parallaxBase);}});$drawerBtnClose.click(function(e){e.preventDefault();$this.closeDrawer($drawer);$this.setParallaxCoords($parallaxBack,$parallaxBase,$header);});},openDrawer:function($drawer){if($drawer.length===0){return 0;}
var $drawerWrapper=$drawer.find('.drawer__wrapper');$this.cache.$body.addClass('drawer-open');if($drawer.hasClass('close')&&$drawerWrapper.length>0){$drawer.removeClass('close').addClass('open');$drawer.css('zIndex','8');$drawerWrapper.one('webkitTransitionEnd transitionend webkitAnimationEnd oanimationend msAnimationEnd animationend',function showIt(){$drawerWrapper.css('display','block');$drawerWrapper.off('webkitTransitionEnd transitionend webkitAnimationEnd oanimationend msAnimationEnd animationend',showIt);});}else if($drawer.hasClass('close')){$drawer.removeClass('close').addClass('open');}},closeDrawer:function($drawer){if($drawer.length===0){return 0;}
var $drawerWrapper=$drawer.find('.drawer__wrapper');if($drawer.hasClass('open')&&$drawerWrapper.length>0){$drawer.removeClass('open').addClass('close');$drawerWrapper.one('webkitTransitionEnd transitionend webkitAnimationEnd oanimationend msAnimationEnd animationend',function hideIt(){$drawerWrapper.css('display','');$drawer.css('zIndex','');$drawerWrapper.off('webkitTransitionEnd transitionend webkitAnimationEnd oanimationend msAnimationEnd animationend',hideIt);});}else if($drawer.hasClass('open')){$drawer.removeClass('open').addClass('close');}
$this.cache.$body.removeClass('drawer-open');},collapseSubmenu:function(drawer){var $drawer=$(drawer);if($drawer.length===0){return 0;}
var collapseToggles=$drawer.find('.js-collapse');collapseToggles.click(function(e){e.preventDefault();$(this).parent('.has-dropdown').toggleClass('show-dropdown');});},searchHeaderToggle:function(){var $searchContainer=$('.search-container');var $searchToggle=$searchContainer.find('.js-icon--search');var $searchForm=$searchContainer.find('form[action="/search"]');$searchToggle.click(function(){if($searchContainer.hasClass('open')){$searchForm.submit();}else{$searchForm.find('input[name="q"]').focus();$searchContainer.addClass('open');}});},initAddToCard:function(forms){var $forms=$(forms);if($forms.length===0){return 0;}
var $this=this;$forms.submit(function(e){e.preventDefault();var $target=$(e.target);var variantId=$target.find('input[name=id]').val();var quantity=$target.find('input[name=quantity]').val()||1;$this.doAjaxAddToCart(variantId,quantity,$(this));});},doAjaxAddToCart:function(variantId,quantity,$_this_){if($_this_.hasClass('wishlist-add-cart')){var $elementForInsert=$('.table-wrapper');}else{var $elementForInsert=$_this_.find('.js-add-to-card');}
$.ajax({type:"post",url:"/cart/add.js",data:'quantity='+quantity+'&id='+variantId,dataType:'json',beforeSend:function(){if(!$_this_.hasClass('js-messages-disable')){$elementForInsert.addClass('loading-spiner');}},success:function(msg){foodly.updateDropdownCart();if(!$_this_.hasClass('js-messages-disable')){var $formMessage=$_this_.find('.form-cart-message');if($formMessage.length>0&&$formMessage.hasClass('hide-message')){$formMessage.removeClass('hide-message hide').addClass('show-message');}else{$formMessage=$('<p/>').addClass('form-cart-message show-message').html('The product was successfully added to cart.');$elementForInsert.before($formMessage);}
...
setTimeout(function(){$formMessage.addClass('hide-message').removeClass('show-message');$formMessage.one('webkitAnimationEnd oanimationend msAnimationEnd animationend',function addClasses(){$formMessage.addClass('hide');$formMessage.off('webkitAnimationEnd oanimationend msAnimationEnd animationend',addClasses);});},7000);$elementForInsert.removeClass('loading-spiner');}});},updateDropdownCart:function(){Shopify.getCart(function(cart){foodly.doUpdateDropdownCart(cart,false);});},doUpdateDropdownCart:function(cart,removeItem){var template='<li class="cart-list__item" id="product-id-{ID}"><div class="cart-list__item--img clearfix"><div class="img-holder img-holder--circle img-holder--circle-sm"><a href="{URL}"><img src="{IMAGE}" alt="{TITLE}"></a></div></div><div class="cart-list__item--title"><a href="{URL}">{TITLE}</a></div><div class="cart-list__item--quantity"><div class="quantity-field"><span class="icon icon--arrow-up js-up-quantity"></span><input id="updates_{ID}" type="number" name="updates[]" min="1" value="{QUANTITY}" size="5"><span class="icon icon--arrow-down js-down-quantity"></span></div></div><div class="cart-list__item--price"><span class="money">{PRICE}</span></div><div class="cart-list__item--remove"><a href="javascript:void(0)" class="js-cart-item-remove"><span class="icon icon--delete"></span></a></div></li>';var specialPackTemplate='<div class="cart-list__item cart-list__item--special" id="product-id-{ID}"><div class="cart-list__item--img clearfix"><div class="img-holder img-holder--circle img-holder--circle-sm"><img src="{IMAGE}" alt="{TITLE}"></div></div><div class="cart-list__item--title">{TITLE}</div><div class="cart-list__item--quantity-special"></div><div class="cart-list__item--price"><span class="money">{PRICE}</span></div><div class="cart-list__item--remove"><a href="javascript:void(0)" class="js-cart-item-remove"></a></div></div>';var specialPack=false;var specialPackID=this.cache.$cart.find('.cart-list__item--special').attr('id');var $this=this;var $cartContainer=this.cache.$cart.find('.cart');var $cartForm=$cartContainer.find('#cart-form');var $cartEmpty=$cartContainer.find('.cart-empty');var $cartCount=this.cache.$cart.find('#cartCount');var $cartHeader=$this.cache.$cart.find('.cart-header');if(specialPackID){specialPackID=specialPackID.match(/\d+/g);}
...
if(removeItem){var $removeMessage=$this.cache.$cart.find('.remove-message');if($removeMessage.length>0&&$removeMessage.hasClass('hide-message')){$removeMessage.removeClass('hide-message hide').addClass('show-message');}else{$removeMessage=$('<div/>').addClass('remove-message show-message');$removeMessageText=$('<p/>').text('The item has been removed');$removeMessage.append($removeMessageText);$cartHeader.after($removeMessage);}
setTimeout(function(){$removeMessage.removeClass('show-message').addClass('hide-message');$removeMessage.one('webkitAnimationEnd oanimationend msAnimationEnd animationend',function addClasses(){$removeMessage.addClass('hide');$removeMessage.off('webkitAnimationEnd oanimationend msAnimationEnd animationend',addClasses);});},2500);}
this.cache.$cart.find('.cart-total__amount .money').html(Shopify.formatMoney(cart.total_price,Shopify.money_format));this.cache.$cart.find('.cart-list').html('');this.initChangeQuantity('.cart');if(cart.item_count>0){var item,truncateTilte,specialPackItem;$cartEmpty.addClass('display-none');$cartForm.removeClass('display-none');for(var i=0,max=cart.items.length;i<max;i++){item=template;specialPackItem=specialPackTemplate;if(cart.items[i].title.length>20){truncateTilte=cart.items[i].title.substr(0,17);truncateTilte=truncateTilte+'...';}
if(specialPackID&&specialPackID[0]==cart.items[i].id){continue;}
if(specialPack&&cart.items[i].sku==""){specialPackItem=specialPackItem.replace(/\{ID\}/g,cart.items[i].id);specialPackItem=specialPackItem.replace(/\{TITLE\}/g,truncateTilte||cart.items[i].title);specialPackItem=specialPackItem.replace(/\{IMAGE\}/g,Shopify.resizeImage(cart.items[i].image,'small'));specialPackItem=specialPackItem.replace(/\{PRICE\}/g,Shopify.formatMoney(cart.items[i].price,Shopify.money_format));$(specialPackItem).insertAfter(this.cache.$cart.find('.cart__package'));continue;}else if(!specialPack&&this.cache.$cart.find('.cart-list__item--special').length>0){this.cache.$cart.find('.cart-list__item--special').remove();}
item=item.replace(/\{ID\}/g,cart.items[i].id);item=item.replace(/\{URL\}/g,cart.items[i].url);item=item.replace(/\{TITLE\}/g,truncateTilte||cart.items[i].title);item=item.replace(/\{QUANTITY\}/g,cart.items[i].quantity);if(cart.items[i].image==null){item=item.replace(/\{IMAGE\}/g,Shopify.resizeImage("//cdn.shopify.com/s/files/1/2168/1709/t/20/assets/img_no_image_sm.jpg?11891598687326378541",'small'));}else{item=item.replace(/\{IMAGE\}/g,Shopify.resizeImage(cart.items[i].image,'compact'));}
item=item.replace(/\{PRICE\}/g,Shopify.formatMoney(cart.items[i].price,Shopify.money_format));this.cache.$cart.find('.cart-list').append(item);}
this.initChangeQuantity('.cart');this.cache.$cart.find('.js-cart-item-remove').click(function(e){e.preventDefault();var productId=$(this).parents('.cart-list__item').attr('id');productId=productId.match(/\d+/g);Shopify.removeItem(productId,function(cart){foodly.doUpdateDropdownCart(cart,true);});$this.initChangeQuantity('.cart');});}else{$cartForm.addClass('display-none');$cartEmpty.removeClass('display-none');}
},initChangeQuantity:function(form){var $form=$(form);var forms=document.querySelectorAll(form);if(forms.length===0){return 0;}
var quantityContainer,arrowUp,arrowDown,inputField,inputFieldValue;for(var i=0,max=forms.length;i<max;i++){quantityContainer=$($form[i]).find('.quantity-field');arrowUp=quantityContainer.find('.js-up-quantity');arrowDown=quantityContainer.find('.js-down-quantity');arrowUp.click(function(e){e.stopPropagation();inputField=$(e.currentTarget).next();inputFieldValue=inputField.val();inputFieldValue++;inputField.val(inputFieldValue);inputField.trigger("change");});arrowDown.click(function(e){inputField=$(e.currentTarget).prev();inputFieldValue=inputField.val();if(inputFieldValue==0){inputFieldValue=0;}else{inputFieldValue--;}
...
var $firstName=$form.find('[name*=first_name]');var $lastName=$form.find('[name*=last_name]');var $email=$form.find('[name*=email]');var $password=$form.find('[name*=password]');var $passwordConfirm=$form.find('[name*=\\[password_confirmation\\]]');var $message=$form.find('[name*=body]');var $fullName=$form.find('[name*=\\[name\\]]');var $author=$form.find('[name*=author]');var $address1=$form.find('[name*=address1]');var $city=$form.find('[name*=city]');var $zip=$form.find('[name*=zip]');var errorMsgName="translation missing: en.general.form_popup_errors.name_html";var errorMsgLastName="translation missing: en.general.form_popup_errors.last_name_html";var errorMsgEmail="translation missing: en.general.form_popup_errors.email_html";var errorMsgPassword="translation missing: en.general.form_popup_errors.password_html";var errorMsgMessage="translation missing: en.general.form_popup_errors.message_html";var errorMsgFullname="translation missing: en.general.form_popup_errors.fullname_html";var errorMsgAuthor="translation missing: en.general.form_popup_errors.comment_author_html";var errorMsgAddress1="translation missing: en.general.form_popup_errors.city_html";var errorMsgCity="translation missing: en.general.form_popup_errors.city_html";var errorMsgZip="translation missing: en.general.form_popup_errors.zip_html";var counter=0;function initErrorMsg(input,msg){for(var i=0,max=$inputs.length;i<max;i++){if(input.is($inputs[i])){counter=i+1;}}
...
var y;var $this=this;var headerHeight=$fixedHeader.outerHeight();var $navList=$fixedHeader.find('.nav-list');var navlistHeight=$navList.outerHeight();var headerHeightMinusNavlist=headerHeight-navlistHeight;var refreshY=$(window).scrollTop();if(refreshY>navlistHeight){checkScrollHeader();}
function checkScrollHeader(){if($this.cache.$body.hasClass('cart-open')){return 0;}
y=$(window).scrollTop();if(y>80&&!$fixedHeader.hasClass('fixed')){$fixedHeader[0].classList.add('fixed');}else if(y<80&&$fixedHeader.hasClass('fixed')){$fixedHeader[0].classList.remove('fixed');}}
$(window).scroll(function(){window.requestAnimationFrame(checkScrollHeader);});},parallaxScroll:function(parallaxContainer){var $parallaxContainer=$(parallaxContainer);if($parallaxContainer.length===0){return 0;}
var _this=this;var $parallaxBack=$parallaxContainer.find('.parallax__back');var parallaxBackHeight=$parallaxBack.outerHeight();var $parallaxBase=$parallaxContainer.find('.parallax__base');var $header=this.cache.$body.find('.header');var headerHeight=$header.outerHeight();var coordsBase=$parallaxBase[0].getBoundingClientRect();var windowBase=($(window).height()-$(window).height()*0.04);var emptyDiv=$('<div/>').addClass('empty-div').css('height',$parallaxBack.height());if($parallaxContainer.parent().hasClass('main-content--top')){$parallaxContainer.parent().css('margin-top','0px');}
...
if(y>200){$parallaxContainer.addClass('parallax-move');}
if(y>coordsBase.bottom-300&&$parallaxContainer.hasClass('parallax--additional')){$parallaxBack.css('opacity','0');}else if(y<coordsBase.bottom-300&&$parallaxContainer.hasClass('parallax--additional')){$parallaxBack.css('opacity','1');}});});},setParallaxCoords:function($parallaxBack,$parallaxBase,$header){if($header.length===0||$parallaxBack.length===0||$parallaxBase.length===0){return 0;}
...
$parallaxBack.css({'transition':'none','-webkit-transition':'none'});$parallaxBase.css({'transition':'none','-webkit-transition':'none'});},disableScroll:function(){if(window.addEventListener)
...
e.preventDefault();e.returnValue=false;}};global.updateCart=foodly.updateDropdownCart;global.coverImg=foodly.coverImg;global.foodly=foodly;return global;})($,global||{});